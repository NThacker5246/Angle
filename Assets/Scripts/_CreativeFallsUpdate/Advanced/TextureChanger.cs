...
public class TextureChanger : MonoBehaviour
{
	...
	public void SetTexture(){
		be.getSelectedBlock().GetComponent<Renderer>().material.color = new Color32((byte)r.value, (byte)g.value, (byte)b.value, 255);
	...
	public void ReadTexture(){
		if(be.getSelectedBlock().GetComponent<Ids>().mainId == 0 && be.getSelectedBlock().GetComponent<Ids>().groupId == 0) {
			Color32 c = be.getSelectedBlock().GetComponent<Renderer>().material.color;
			r.value = (float) c.r;
			g.value = (float) c.g;
			b.value = (float) c.b;
		}
